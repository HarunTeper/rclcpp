...
# NOTE (humble): memory_strategy_ era — no EventsCBGExecutor, and humble ships the reduced
# benchmark_executor (spin_some only). compare-humble measures fixed-vs-unfixed MTE
# (allocations + timing) + the starvation verdict; the EventsCBG column is N/A here.
compare-humble:
	bash docker/run-comparison.sh humble fix/mte-starvation-humble upstream/humble $(BENCH_REPS)
...
.PHONY: compare-humble compare-lyrical compare-jazzy autoware-build autoware-smoke autoware-shell \
        autoware-lyrical-build autoware-lyrical-smoke
